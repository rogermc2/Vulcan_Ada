...
package Vulkan.C_Arrays is
    ...
    -- Get the array associated with a pointer.
    function Get_Array(P: in Pointer) return Array_Access
        with Post => (P = null) = (Get_Array'Result = null);
    ...
    procedure Free(P: in out Pointer; Release_Element: in Free_Element)
        with Post => P = null;
    procedure Free(A: in out Array_Access)
        with Post => A = null;
end Vulkan.C_Arrays;
