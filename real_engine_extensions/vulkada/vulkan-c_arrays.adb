...
package body Vulkan.C_Arrays is
    ...
    procedure Free_Array is new Ada.Unchecked_Deallocation(Element_Array,
                                                           Array_Access);
    ...
    protected Allocations is
        ...
        procedure Free(Array_Access: in C_Arrays.Array_Access);
        function Get_Array(Pointer: in C_Arrays.Pointer) return Array_Access;
    private
    ...
    function Get_Array(P: in Pointer) return Array_Access is
    begin
        if P = null then
            return null;
        end if;

        return Allocations.Get_Array(P);
    end Get_Array;
    ...
    procedure Free(A: in out Array_Access) is
    begin
        if A /= null then
            Allocations.Free(A);
        end if;
    ...
    begin
        if Left.Pointer /= null and Right.Pointer /= null then
            return Left.Pointer = Right.Pointer;
        end if;

        return Left.Array_Access = Right.Array_Access;
    end "=";
    ...
    protected body Allocations is
        ...
        procedure Free(Pointer: in C_Arrays.Pointer) is
            ...
            begin
                Free_Array(Element.Array_Access);
            end Release;
        ...
        procedure Free(Array_Access: in C_Arrays.Array_Access) is
            Dummy_Record: Allocation_Record;
            Position: Allocation_Lists.Cursor;
        begin
            Dummy_Record.Array_Access := Array_Access;
            Position := Records.Find(Dummy_Record);
            Free_Array(Dummy_Record.Array_Access);
            Records.Delete(Position);
        end Free;

        function Get_Array(Pointer: in C_Arrays.Pointer) return Array_Access is
            Dummy_Record: Allocation_Record;
            Position: Allocation_Lists.Cursor;
        begin
            Dummy_Record.Pointer := Pointer;
            ...
            return Allocation_Lists.Element(Position).Array_Access;
        end Get_Array;
    end Allocations;
end Vulkan.C_Arrays;
