...
package body Vulkan.Devices is
    ...
    -- Storage for device memory report callbacks.
    package Memory_Report_Marshallers is
        new Callback_Marshallers(Device,
                                 Interfaces.C.Extensions.void_ptr,
                                 Extensions.EXT.Device_Memory_Report_Callback,
                                 Vulkan."<");
    
    -- Device memory report dispatcher.
    procedure Memory_Report_Callback_Proxy
        (Callback_Data: in C_EXT.Device_Memory_Report_Callback_Data_C;
         User_Data: in Interfaces.C.Extensions.void_ptr)
        with Convention => C;

    Memory_Report_Callbacks: Memory_Report_Marshallers.Marshallers;
    ...
    begin
        return Create(Physical_Device, Create_Info, Allocator'Access, Device);
    end Create;
    ...
    function Create(Physical_Device: in Vulkan.Physical_Device;
                    ...
                    Device: out Vulkan.Device) return Result is
    begin
        return Create(Physical_Device, Create_Info, null, Device);
    end Create;
    ...
    function Create(Physical_Device: in Vulkan.Physical_Device;
        ...
        D: Device;
    begin
        Exceptions.Check(Create(Physical_Device, Create_Info, null, D));
    ...
    begin
        ...
        Memory_Report_Callbacks.Remove(Device);
        Device := No_Device;
    end Destroy;
    ...
    function Map_With_Result_Flags(Device: in Vulkan.Device;
                                   Memory: in Device_Memory;
                                   Offset, Size: in Device_Size;
                                   Flags: in Memory_Map_Flags;
                                   Data: out Pointers.Pointer) return Result is
    begin
        return C.vkMapMemory(Device, Memory, Offset, Size, Flags, Data'Address);
    end Map_With_Result_Flags;
    ...
    function Map_With_Exception_Flags(Device: in Vulkan.Device;
                                      Memory: in Device_Memory;
                                      Flags: in Memory_Map_Flags;
                                      Offset, Size: in Device_Size)
        return Pointers.Pointer is
    ...
    function Create(Physical_Device: in Vulkan.Physical_Device;
                    ...
                    Allocator: access constant Allocation_Callbacks;
                    Device: out Vulkan.Device) return Result is
        package Memory_Report_Vectors is new Ada.Containers.Vectors
            (Positive,
             Memory_Report_Marshallers.Marshaller_Access,
             Memory_Report_Marshallers."=");
        ...
        Memory_Reports: Memory_Report_Vectors.Vector;
        Result: Vulkan.Result;
    begin
        ...
        declare
            Extension: In_Structure_Access := Create_Info.Next;
            Extension_C: C.In_Structure_C_Access := Create_Info_C.Next;

            procedure Register_Device_Memory_Report is
                function To_C_Report is new Ada.Unchecked_Conversion
                   (C.In_Structure_C_Access,
                    C_EXT.Device_Device_Memory_Report_Create_Info_C_Access);
                ...
                Report_Info_C:
                    C_EXT.Device_Device_Memory_Report_Create_Info_C_Access
                        := To_C_Report(Extension_C);
                Marshaller: Memory_Report_Marshallers.Marshaller_Access :=
                    new Memory_Report_Marshallers.Marshaller;
            begin
                Marshaller.Data := Report_Info.User_Data;
                Marshaller.Callback := Report_Info.User_Callback;
                Report_Info_C.User_Callback :=
                    Memory_Report_Callback_Proxy'Access;
                Report_Info_C.User_Data := Marshaller.all'Address;
                Memory_Reports.Append(Marshaller);
            end Register_Device_Memory_Report;
        begin
            while Extension /= null loop
                if Extension.all in
                 Extensions.EXT.Device_Device_Memory_Report_Create_Info then
                    Register_Device_Memory_Report;
                end if;

                Extension := Extension.Next;
                Extension_C := Extension_C.Next;
            end loop;
        end;
        ...
        if Result = Success then
            for Report of Memory_Reports loop
                Memory_Report_Callbacks.Register(Device, Report);
            end loop;
        else
            for Report of Memory_Reports loop
                Memory_Report_Marshallers.Free(Report);
            end loop;
        end if;
    ...
    procedure Memory_Report_Callback_Proxy
        (Callback_Data: in C_EXT.Device_Memory_Report_Callback_Data_C;
         User_Data: in Interfaces.C.Extensions.void_ptr) is
        Ada_Data: Extensions.EXT.Device_Memory_Report_Callback_Data;
        Marshaller: Memory_Report_Marshallers.Marshaller_Access :=
            Memory_Report_Marshallers.To_Marshaller(User_Data);
    begin
        C_EXT.To_Ada(Ada_Data, Callback_Data);
        Marshaller.Callback(Ada_Data, Marshaller.Data);
    end Memory_Report_Callback_Proxy;
end Vulkan.Devices;
