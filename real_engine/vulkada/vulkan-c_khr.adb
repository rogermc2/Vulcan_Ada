...
package body Vulkan.C_KHR is
    ...
    function To_C(Struct: in Extensions.KHR.Present_ID) return Present_ID_C is
        procedure To_C_Array is
            new Utilities.To_C_Array(C.Uint64_t_Arrays, Unsigned_64_Vectors);
    ...
    begin
        Extension_Records.Free(Struct.Next);
        C.Uint64_t_Arrays.Free(Struct.Present_IDs);
    end Free;
   ...
   Extensions.KHR.Physical_Device_Shader_Subgroup_Uniform_Control_Flow_Features;
    ...
    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Pipeline_Binary_Features;
         C_Struct: in Physical_Device_Pipeline_Binary_Features_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Pipeline_Binaries :=
            Utilities.To_Ada(C_Struct.Pipeline_Binaries);
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Pipeline_Binary_Properties;
         C_Struct: in Physical_Device_Pipeline_Binary_Properties_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Pipeline_Binary_Internal_Cache :=
            Utilities.To_Ada(C_Struct.Pipeline_Binary_Internal_Cache);
        Ada_Struct.Pipeline_Binary_Internal_Cache_Control :=
            Utilities.To_Ada(C_Struct.Pipeline_Binary_Internal_Cache_Control);
        Ada_Struct.Pipeline_Binary_Prefers_Internal_Cache :=
            Utilities.To_Ada(C_Struct.Pipeline_Binary_Prefers_Internal_Cache);
        Ada_Struct.Pipeline_Binary_Precompiled_Internal_Cache :=
            Utilities.To_Ada
                (C_Struct.Pipeline_Binary_Precompiled_Internal_Cache);
        Ada_Struct.Pipeline_Binary_Compressed_Data :=
            Utilities.To_Ada(C_Struct.Pipeline_Binary_Compressed_Data);
    end To_Ada;

    function To_C
        (Struct: in
            Extensions.KHR.Device_Pipeline_Binary_Internal_Cache_Control)
        return Device_Pipeline_Binary_Internal_Cache_Control_C is
        DPBICCC: Device_Pipeline_Binary_Internal_Cache_Control_C;
    begin
        DPBICCC.Next := Extension_Records.To_C(Struct.Next);
        DPBICCC.Disable_Internal_Cache :=
            Utilities.To_C(Struct.Disable_Internal_Cache);

        return DPBICCC;
    end To_C;

    procedure Free
        (Struct: in out Device_Pipeline_Binary_Internal_Cache_Control_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;

    procedure To_Ada(Ada_Struct: in out Extensions.KHR.Pipeline_Binary_Key;
                     C_Struct: in Pipeline_Binary_Key_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Key_Size := C_Struct.Key_Size;
        Ada_Struct.Key := C_Struct.Key;
    end To_Ada;

    function To_C(Struct: in Extensions.KHR.Pipeline_Binary_Keys_And_Data)
        return Pipeline_Binary_Keys_And_Data_C is
        function To_C(Struct: in Extensions.KHR.Pipeline_Binary_Key)
            return Pipeline_Binary_Key_C is
            PBKC: Pipeline_Binary_Key_C;
        begin
            PBKC.Key_Size := Struct.Key_Size;
            PBKC.Key := Struct.Key;
        ...
        procedure To_C_Array is new Utilities.To_C_Array_Convert
            (Pipeline_Binary_Key_C_Arrays,
             Extensions.KHR.Pipeline_Binary_Key_Vectors);
        procedure To_C_Array is new Utilities.To_C_Array
            (Pipeline_Binary_Data_Arrays,
             Extensions.KHR.Pipeline_Binary_Data_Vectors);
        
        PBKADC: Pipeline_Binary_Keys_And_Data_C;
    begin
        To_C_Array(PBKADC.Binary_Count,
                   Struct.Pipeline_Binary_Keys,
                   PBKADC.Pipeline_Binary_Keys);
        To_C_Array(PBKADC.Binary_Count,
                   Struct.Pipeline_Binary_Data,
                   PBKADC.Pipeline_Binary_Data);

        return PBKADC;
    end To_C;

    procedure Free(Struct: in out Pipeline_Binary_Keys_And_Data_C) is
    begin
        Pipeline_Binary_Key_C_Arrays.Free(Struct.Pipeline_Binary_Keys);
        Pipeline_Binary_Data_Arrays.Free(Struct.Pipeline_Binary_Data);
    end Free;

    function To_C(Struct: in Extensions.KHR.Pipeline_Create_Info)
        return Pipeline_Create_Info_C is
        PCIP: Pipeline_Create_Info_C;
    begin
        PCIP.Next := Extension_Records.To_C(Struct.Next);

        return PCIP;
    end To_C;

    procedure Free(Struct: in out Pipeline_Create_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;

    function To_C(Struct: in Extensions.KHR.Pipeline_Binary_Create_Info)
        return Pipeline_Binary_Create_Info_C is
        use type Extensions.KHR.Pipeline_Binary_Keys_And_Data_Access;
        use type Extensions.KHR.Pipeline_Create_Info_Access;

        PBCIC: Pipeline_Binary_Create_Info_C;
    begin
        PBCIC.Next := Extension_Records.To_C(Struct.Next);
        PBCIC.Pipeline := Struct.Pipeline;

        if Struct.Keys_And_Data_Info /= null then
            PBCIC.Keys_And_Data_Info :=
                new Pipeline_Binary_Keys_And_Data_C'
                    (To_C(Struct.Keys_And_Data_Info.all));
        end if;

        if Struct.Pipeline_Create_Info /= null then
            PBCIC.Pipeline_Create_Info :=
                new Pipeline_Create_Info_C'
                    (To_C(Struct.Pipeline_Create_Info.all));
        end if;

        return PBCIC;
    end To_C;

    procedure Free(Struct: in out Pipeline_Binary_Create_Info_C) is
        procedure Free is
            new Ada.Unchecked_Deallocation
                (Pipeline_Binary_Keys_And_Data_C,
                 Pipeline_Binary_Keys_And_Data_C_Access);
        procedure Free is
            new Ada.Unchecked_Deallocation(Pipeline_Create_Info_C,
                                           Pipeline_Create_Info_C_Access);
    begin
        Extension_Records.Free(Struct.Next);

        if Struct.Keys_And_Data_Info /= null then
            Free(Struct.Keys_And_Data_Info.all);
            Free(Struct.Keys_And_Data_Info);
        end if;

        if Struct.Pipeline_Create_Info /= null then
            Free(Struct.Pipeline_Create_Info.all);
            Free(Struct.Pipeline_Create_Info);
        end if;
    end Free;
                  
    function To_C(Struct: in Extensions.KHR.Pipeline_Binary_Info)
        return Pipeline_Binary_Info_C is
        procedure To_C_Array is
            new Utilities.To_C_Array(Pipeline_Binary_Arrays,
                                     Extensions.KHR.Pipeline_Binary_Vectors);

        PBIC: Pipeline_Binary_Info_C;
    begin
        PBIC.Next := Extension_Records.To_C(Struct.Next);
        To_C_Array(PBIC.Binary_Count,
                   Struct.Pipeline_Binaries,
                   PBIC.Pipeline_Binaries);

        return PBIC;
    end To_C;

    procedure Free(Struct: in out Pipeline_Binary_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
        Pipeline_Binary_Arrays.Free(Struct.Pipeline_Binaries);
    end Free;

    procedure To_Ada
        (Ada_Struct: in out Extensions.KHR.Release_Captured_Pipeline_Data_Info;
         C_Struct: in Release_Captured_Pipeline_Data_Info_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Pipeline := C_Struct.Pipeline;
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out Extensions.KHR.Pipeline_Binary_Data_Info;
         C_Struct: in Pipeline_Binary_Data_Info_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Pipeline_Binary := C_Struct.Pipeline_Binary;
    end To_Ada;

    function To_C(Struct: in Extensions.KHR.Pipeline_Binary_Handles_Info)
        return Pipeline_Binary_Handles_Info_C is
        procedure To_C_Array is new Utilities.To_C_Array
            (Pipeline_Binary_Arrays, Extensions.KHR.Pipeline_Binary_Vectors);

        PBHIC: Pipeline_Binary_Handles_Info_C;
    begin
        PBHIC.Next := Extension_Records.To_C(Struct.Next);
        To_C_Array(PBHIC.Pipeline_Binary_Count,
                   Struct.Pipeline_Binaries,
                   PBHIC.Pipeline_Binaries);

        return PBHIC;
    end To_C;

    procedure Free(Struct: in out Pipeline_Binary_Handles_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
        Pipeline_Binary_Arrays.Free(Struct.Pipeline_Binaries);
    end Free;
    ...
    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Compute_Shader_Derivatives_Features;
         C_Struct: in Physical_Device_Compute_Shader_Derivatives_Features_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Compute_Derivative_Group_Quads :=
            Utilities.To_Ada(C_Struct.Compute_Derivative_Group_Quads);
        Ada_Struct.Compute_Derivative_Group_Linear :=
            Utilities.To_Ada(C_Struct.Compute_Derivative_Group_Linear);
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
           Extensions.KHR.Physical_Device_Compute_Shader_Derivatives_Properties;
         C_Struct:
            in Physical_Device_Compute_Shader_Derivatives_Properties_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Mesh_And_Task_Shader_Derivatives :=
            Utilities.To_Ada(C_Struct.Mesh_And_Task_Shader_Derivatives);
    end To_Ada;
    ...
    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Video_Encode_AV1_Features;
         C_Struct: in Physical_Device_Video_Encode_AV1_Features_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Video_Encode_AV1 :=
            Utilities.To_Ada(C_Struct.Video_Encode_AV1);
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out Extensions.KHR.Video_Encode_AV1_Capabilities;
         C_Struct: in Video_Encode_AV1_Capabilities_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Flags := C_Struct.Flags;
        Ada_Struct.Max_Level := C_Struct.Max_Level;
        Ada_Struct.Coded_Picture_Alignment := C_Struct.Coded_Picture_Alignment;
        Ada_Struct.Max_Tiles := C_Struct.Max_Tiles;
        Ada_Struct.Min_Tile_Size := C_Struct.Min_Tile_Size;
        Ada_Struct.Max_Tile_Size := C_Struct.Max_Tile_Size;
        Ada_Struct.Superblock_Sizes := C_Struct.Superblock_Sizes;
        Ada_Struct.Max_Single_Reference_Count :=
            C_Struct.Max_Single_Reference_Count;
        Ada_Struct.Single_Reference_Name_Mask :=
            C_Struct.Single_Reference_Name_Mask;
        Ada_Struct.Max_Unidirectional_Compound_Reference_Count :=
            C_Struct.Max_Unidirectional_Compound_Reference_Count;
        Ada_Struct.Max_Unidirectional_Compound_Group_1_Reference_Count :=
            C_Struct.Max_Unidirectional_Compound_Group_1_Reference_Count;
        Ada_Struct.Unidirectional_Compound_Reference_Name_Mask :=
            C_Struct.Unidirectional_Compound_Reference_Name_Mask;
        Ada_Struct.Max_Bidirectional_Compound_Reference_Count :=
            C_Struct.Max_Bidirectional_Compound_Reference_Count;
        Ada_Struct.Max_Bidirectional_Compound_Group_1_Reference_Count :=
            C_Struct.Max_Bidirectional_Compound_Group_1_Reference_Count;
        Ada_Struct.Max_Bidirectional_Compound_Group_2_Reference_Count :=
            C_Struct.Max_Bidirectional_Compound_Group_2_Reference_Count;
        Ada_Struct.Bidirectional_Compound_Reference_Name_Mask :=
            C_Struct.Bidirectional_Compound_Reference_Name_Mask;
        Ada_Struct.Max_Temporal_Layer_Count :=
            C_Struct.Max_Temporal_Layer_Count;
        Ada_Struct.Max_Spatial_Layer_Count := C_Struct.Max_Spatial_Layer_Count;
        Ada_Struct.Max_Operating_Points := C_Struct.Max_Operating_Points;
        Ada_Struct.Min_Q_Index := C_Struct.Min_Q_Index;
        Ada_Struct.Max_Q_Index := C_Struct.Max_Q_Index;
        Ada_Struct.Prefers_GOP_Remaining_Frames :=
            Utilities.To_Ada(C_Struct.Prefers_GOP_Remaining_Frames);
        Ada_Struct.Requires_GOP_Remaining_Frames :=
            Utilities.To_Ada(C_Struct.Requires_GOP_Remaining_Frames);
    ...
    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Video_Encode_AV1_Quality_Level_Properties;
         C_Struct: in Video_Encode_AV1_Quality_Level_Properties_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Preferred_Rate_Control_Flags :=
            C_Struct.Preferred_Rate_Control_Flags;
        Ada_Struct.Preferred_GOP_Frame_Count :=
            C_Struct.Preferred_GOP_Frame_Count;
        Ada_Struct.Preferred_Key_Frame_Period :=
            C_Struct.Preferred_Key_Frame_Period;
        Ada_Struct.Preferred_Consecutive_Bipredictive_Frame_Count :=
            C_Struct.Preferred_Consecutive_Bipredictive_Frame_Count;
        Ada_Struct.Preferred_Temporal_Layer_Count :=
            C_Struct.Preferred_Temporal_Layer_Count;
        Ada_Struct.Preferred_Constant_Q_Index :=
            C_Struct.Preferred_Constant_Q_Index;
        Ada_Struct.Preferred_Max_Single_Reference_Count :=
            C_Struct.Preferred_Max_Single_Reference_Count;
        Ada_Struct.Preferred_Single_Reference_Name_Mask :=
            C_Struct.Preferred_Single_Reference_Name_Mask;
        Ada_Struct.Preferred_Max_Unidirectional_Compound_Reference_Count :=
            C_Struct.Preferred_Max_Unidirectional_Compound_Reference_Count;
        Ada_Struct.Preferred_Max_Unidirectional_Compound_Group_1_Reference_Count
            :=
        C_Struct.Preferred_Max_Unidirectional_Compound_Group_1_Reference_Count;
        Ada_Struct.Preferred_Unidirectional_Compound_Reference_Name_Mask :=
            C_Struct.Preferred_Unidirectional_Compound_Reference_Name_Mask;
        Ada_Struct.Preferred_Max_Bidirectional_Compound_Reference_Count :=
            C_Struct.Preferred_Max_Bidirectional_Compound_Reference_Count;
        Ada_Struct.Preferred_Max_Bidirectional_Compound_Group_1_Reference_Count
            :=
        C_Struct.Preferred_Max_Bidirectional_Compound_Group_1_Reference_Count;
        Ada_Struct.Preferred_Max_Bidirectional_Compound_Group_2_Reference_Count
            :=
        C_Struct.Preferred_Max_Bidirectional_Compound_Group_2_Reference_Count;
        Ada_Struct.Preferred_Bidirectional_Compound_Reference_Name_Mask :=
            C_Struct.Preferred_Bidirectional_Compound_Reference_Name_Mask;
    end To_Ada;

    function To_C
        (Struct: in Extensions.KHR.Video_Encode_AV1_Session_Create_Info)
        return Video_Encode_AV1_Session_Create_Info_C is
        VEASCIC: Video_Encode_AV1_Session_Create_Info_C;
    begin
        VEASCIC.Next := Extension_Records.To_C(Struct.Next);
        VEASCIC.Use_Max_Level := Utilities.To_C(Struct.Use_Max_Level);
        VEASCIC.Max_Level := Struct.Max_Level;

        return VEASCIC;
    end To_C;

    procedure Free(Struct: in out Video_Encode_AV1_Session_Create_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;
    ...
    function To_C
        (Struct: in
            Extensions.KHR.Video_Encode_AV1_Session_Parameters_Create_Info)
        return Video_Encode_AV1_Session_Parameters_Create_Info_C is
        procedure To_C_Array is new Utilities.To_C_Array
            (AV1_Encode_Operating_Point_Info_Arrays,
             Extensions.Std_Video.AV1.Encode.Operating_Point_Info_Vectors);

        VEASPCIC: Video_Encode_AV1_Session_Parameters_Create_Info_C;
    begin
        VEASPCIC.Next := Extension_Records.To_C(Struct.Next);
        VEASPCIC.Std_Sequence_Header := Struct.Std_Sequence_Header;
        VEASPCIC.Std_Decoder_Model_Info := Struct.Std_Decoder_Model_Info;
        To_C_Array(VEASPCIC.Std_Operating_Point_Count,
                   Struct.Std_Operating_Points,
                   VEASPCIC.Std_Operating_Points);

        return VEASPCIC;
    end To_C;

    procedure Free
        (Struct: in out Video_Encode_AV1_Session_Parameters_Create_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
        AV1_Encode_Operating_Point_Info_Arrays.Free
            (Struct.Std_Operating_Points);
    end Free;

    function To_C(Struct: in Extensions.KHR.Video_Encode_AV1_Picture_Info)
        return Video_Encode_AV1_Picture_Info_C is
        VEAPIC: Video_Encode_AV1_Picture_Info_C;
    begin
        VEAPIC.Next := Extension_Records.To_C(Struct.Next);
        VEAPIC.Prediction_Mode := Struct.Prediction_Mode;
        VEAPIC.Rate_Control_Group := Struct.Rate_Control_Group;
        VEAPIC.Constant_Q_Index := Struct.Constant_Q_Index;
        VEAPIC.Std_Picture_Info := Struct.Std_Picture_Info;
        VEAPIC.Reference_Name_Slot_Indices :=
            Struct.Reference_Name_Slot_Indices;
        VEAPIC.Primary_Reference_CDF_Only :=
            Utilities.To_C(Struct.Primary_Reference_CDF_Only);
        VEAPIC.Generate_OBU_Extension_Header :=
            Utilities.To_C(Struct.Generate_OBU_Extension_Header);

        return VEAPIC;
    end To_C;

    procedure Free(Struct: in out Video_Encode_AV1_Picture_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;

    function To_C(Struct: in Extensions.KHR.Video_Encode_AV1_DPB_Slot_Info)
        return Video_Encode_AV1_DPB_Slot_Info_C is
        VEADSIC: Video_Encode_AV1_DPB_Slot_Info_C;
    begin
        VEADSIC.Next := Extension_Records.To_C(Struct.Next);
        VEADSIC.Std_Reference_Info := Struct.Std_Reference_Info;

        return VEADSIC;
    end To_C;

    procedure Free(Struct: in out Video_Encode_AV1_DPB_Slot_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;

    function To_C(Struct: in Extensions.KHR.Video_Encode_AV1_Profile_Info)
        return Video_Encode_AV1_Profile_Info_C is
        VEAPIC: Video_Encode_AV1_Profile_Info_C;
    begin
        VEAPIC.Next := Extension_Records.To_C(Struct.Next);
        VEAPIC.Std_Profile := Struct.Std_Profile;

        return VEAPIC;
    end To_C;

    procedure Free(Struct: in out Video_Encode_AV1_Profile_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;

    function To_C
        (Struct: in Extensions.KHR.Video_Encode_AV1_GOP_Remaining_Frame_Info)
        return Video_Encode_AV1_GOP_Remaining_Frame_Info_C is
        VEAGRFIC: Video_Encode_AV1_GOP_Remaining_Frame_Info_C;
    begin
        VEAGRFIC.Next := Extension_Records.To_C(Struct.Next);
        VEAGRFIC.Use_GOP_Remaining_Frames :=
            Utilities.To_C(Struct.Use_GOP_Remaining_Frames);
        VEAGRFIC.GOP_Remaining_Intra := Struct.GOP_Remaining_Intra;
        VEAGRFIC.GOP_Remaining_Predictive := Struct.GOP_Remaining_Predictive;
        VEAGRFIC.GOP_Remaining_Bipredictive :=
            Struct.GOP_Remaining_Bipredictive;

        return VEAGRFIC;
    end To_C;

    procedure Free
        (Struct: in out Video_Encode_AV1_GOP_Remaining_Frame_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;

    function To_C(Struct: in Extensions.KHR.Video_Encode_AV1_Rate_Control_Info)
        return Video_Encode_AV1_Rate_Control_Info_C is
        VEARCIC: Video_Encode_AV1_Rate_Control_Info_C;
    begin
        VEARCIC.Next := Extension_Records.To_C(Struct.Next);
        VEARCIC.Flags := Struct.Flags;
        VEARCIC.GOP_Frame_Count := Struct.GOP_Frame_Count;
        VEARCIC.Key_Frame_Period := Struct.Key_Frame_Period;
        VEARCIC.Consecutive_Bipredictive_Frame_Count :=
            Struct.Consecutive_Bipredictive_Frame_Count;
        VEARCIC.Temporal_Layer_Count := Struct.Temporal_Layer_Count;

        return VEARCIC;
    end To_C;

    procedure Free(Struct: in out Video_Encode_AV1_Rate_Control_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;

    function To_C
        (Struct: in Extensions.KHR.Video_Encode_AV1_Rate_Control_Layer_Info)
        return Video_Encode_AV1_Rate_Control_Layer_Info_C is
        VEARCLIC: Video_Encode_AV1_Rate_Control_Layer_Info_C;
    begin
        VEARCLIC.Next := Extension_Records.To_C(Struct.Next);
        VEARCLIC.Use_Min_Q_Index := Utilities.To_C(Struct.Use_Min_Q_Index);
        VEARCLIC.Min_Q_Index := Struct.Min_Q_Index;
        VEARCLIC.Use_Max_Q_Index := Utilities.To_C(Struct.Use_Max_Q_Index);
        VEARCLIC.Max_Q_Index := Struct.Max_Q_Index;
        VEARCLIC.Use_Max_Frame_Size :=
            Utilities.To_C(Struct.Use_Max_Frame_Size);
        VEARCLIC.Max_Frame_Size := Struct.Max_Frame_Size;

        return VEARCLIC;
    end To_C;

    procedure Free(Struct: in out Video_Encode_AV1_Rate_Control_Layer_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;
    ...
    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Video_Encode_Quantization_Map_Capabilities;
         C_Struct: in Video_Encode_Quantization_Map_Capabilities_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Max_Quantization_Map_Extent :=
            C_Struct.Max_Quantization_Map_Extent;
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Video_Format_Quantization_Map_Properties;
         C_Struct: in Video_Format_Quantization_Map_Properties_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Quantization_Map_Texel_Size :=
            C_Struct.Quantization_Map_Texel_Size;
    end To_Ada;

    function To_C(Struct: in Extensions.KHR.Video_Encode_Quantization_Map_Info)
        return Video_Encode_Quantization_Map_Info_C is
        VEQMIC: Video_Encode_Quantization_Map_Info_C;
    begin
        VEQMIC.Next := Extension_Records.To_C(Struct.Next);
        VEQMIC.Quantization_Map := Struct.Quantization_Map;
        VEQMIC.Quantization_Map_Extent := Struct.Quantization_Map_Extent;

        return VEQMIC;
    end To_C;

    procedure Free(Struct: in out Video_Encode_Quantization_Map_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;
    ...
    function To_C
        (Struct: in
    Extensions.KHR.Video_Encode_Quantization_Map_Session_Parameters_Create_Info)
        return Video_Encode_Quantization_Map_Session_Parameters_Create_Info_C is
        VEQMSPCIC:
            Video_Encode_Quantization_Map_Session_Parameters_Create_Info_C;
    begin
        VEQMSPCIC.Next := Extension_Records.To_C(Struct.Next);
        VEQMSPCIC.Quantization_Map_Texel_Size :=
            Struct.Quantization_Map_Texel_Size;
    ...
    procedure Free
        (Struct: in out
            Video_Encode_Quantization_Map_Session_Parameters_Create_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;
    ...
    procedure To_Ada
        (Ada_Struct: in out
          Extensions.KHR.Physical_Device_Video_Encode_Quantization_Map_Features;
         C_Struct: in
            Physical_Device_Video_Encode_Quantization_Map_Features_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Video_Encode_Quantization_Map :=
            Utilities.To_Ada(C_Struct.Video_Encode_Quantization_Map);
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Video_Encode_H264_Quantization_Map_Capabilities;
         C_Struct: in Video_Encode_H264_Quantization_Map_Capabilities_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Min_QP_Delta := C_Struct.Min_QP_Delta;
        Ada_Struct.Max_QP_Delta := C_Struct.Max_QP_Delta;
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Video_Encode_H265_Quantization_Map_Capabilities;
         C_Struct: in Video_Encode_H265_Quantization_Map_Capabilities_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Min_QP_Delta := C_Struct.Min_QP_Delta;
        Ada_Struct.Max_QP_Delta := C_Struct.Max_QP_Delta;
    ...
    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Video_Format_H265_Quantization_Map_Properties;
         C_Struct: in Video_Format_H265_Quantization_Map_Properties_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Compatible_CTB_Sizes := C_Struct.Compatible_CTB_Sizes;
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Video_Encode_AV1_Quantization_Map_Capabilities;
         C_Struct: in Video_Encode_AV1_Quantization_Map_Capabilities_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Min_Q_Index_Delta := C_Struct.Min_Q_Index_Delta;
        Ada_Struct.Max_Q_Index_Delta := C_Struct.Max_Q_Index_Delta;
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Video_Format_AV1_Quantization_Map_Properties;
         C_Struct: in Video_Format_AV1_Quantization_Map_Properties_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Compatible_Superblock_Sizes :=
            C_Struct.Compatible_Superblock_Sizes;
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
    Extensions.KHR.Physical_Device_Shader_Relaxed_Extended_Instruction_Features;
         C_Struct: in
            Physical_Device_Shader_Relaxed_Extended_Instruction_Features_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Shader_Relaxed_Extended_Instruction :=
            Utilities.To_Ada(C_Struct.Shader_Relaxed_Extended_Instruction);
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Maintenance_7_Features;
         C_Struct: in Physical_Device_Maintenance_7_Features_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Maintenance_7 := Utilities.To_Ada(C_Struct.Maintenance_7);
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Maintenance_7_Properties;
         C_Struct: in Physical_Device_Maintenance_7_Properties_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Robust_Fragment_Shading_Rate_Attachment_Access :=
            Utilities.To_Ada
                (C_Struct.Robust_Fragment_Shading_Rate_Attachment_Access);
        Ada_Struct.Separate_Depth_Stencil_Attachment_Access :=
            Utilities.To_Ada
                (C_Struct.Separate_Depth_Stencil_Attachment_Access);
        Ada_Struct.Max_Descriptor_Set_Total_Uniform_Buffers_Dynamic :=
            C_Struct.Max_Descriptor_Set_Total_Uniform_Buffers_Dynamic;
        Ada_Struct.Max_Descriptor_Set_Total_Storage_Buffers_Dynamic :=
            C_Struct.Max_Descriptor_Set_Total_Storage_Buffers_Dynamic;
        Ada_Struct.Max_Descriptor_Set_Total_Buffers_Dynamic :=
            C_Struct.Max_Descriptor_Set_Total_Buffers_Dynamic;
        Ada_Struct.
        ...
        Ada_Struct.
            ...
            C_Struct.
             Max_Descriptor_Set_Update_After_Bind_Total_Storage_Buffers_Dynamic;
        Ada_Struct.Max_Descriptor_Set_Update_After_Bind_Total_Buffers_Dynamic :=
            C_Struct.Max_Descriptor_Set_Update_After_Bind_Total_Buffers_Dynamic;
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Layered_API_Properties;
         C_Struct: in Physical_Device_Layered_API_Properties_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Vendor_ID := C_Struct.Vendor_ID;
        Ada_Struct.Device_ID := C_Struct.Device_ID;
        Ada_Struct.Layered_API := C_Struct.Layered_API;
        Ada.Strings.Unbounded.Set_Unbounded_String
            (Ada_Struct.Device_Name,
             Interfaces.C.To_Ada(C_Struct.Device_Name));
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Layered_API_Properties_List;
         C_Struct: in Physical_Device_Layered_API_Properties_List_C) is
        use type Physical_Device_Layered_API_Properties_C_Arrays.Pointer;
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);

        if C_Struct.Layered_APIs = null then
            Ada_Struct.Layered_API_Count := C_Struct.Layered_API_Count;
        else
            declare
                APIs_Array:
                    Physical_Device_Layered_API_Properties_C_Arrays.Array_Access
                        :=
                    Physical_Device_Layered_API_Properties_C_Arrays.Get_Array
                        (C_Struct.Layered_APIs);
            begin
                Ada_Struct.Layered_APIs.Set_Length
                    (Ada.Containers.Count_Type(C_Struct.Layered_API_Count));

                for X in APIs_Array.all'Range loop
                    To_Ada(Ada_Struct.Layered_APIs(X), APIs_Array(X));
                end loop;
                
                Physical_Device_Layered_API_Properties_C_Arrays.Free
                    (APIs_Array);
            end;
        end if;
    end To_Ada;
    ...
    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Layered_API_Vulkan_Properties;
         C_Struct: in Physical_Device_Layered_API_Vulkan_Properties_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        C_V1_1.To_Ada(Ada_Struct.Properties, C_Struct.Properties);
    end To_Ada;

    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Maintenance_8_Features;
         C_Struct: in Physical_Device_Maintenance_8_Features_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Maintenance_8 := Utilities.To_Ada(C_Struct.Maintenance_8);
    end To_Ada;

    function To_C(Struct: in Extensions.KHR.Memory_Barrier_Access_Flags_3)
        return Memory_Barrier_Access_Flags_3_C is
        MBAF3C: Memory_Barrier_Access_Flags_3_C;
    begin
        MBAF3C.Next := Extension_Records.To_C(Struct.Next);
        MBAF3C.Src_Access_Mask_3 := Struct.Src_Access_Mask_3;
        MBAF3C.Dst_Access_Mask_3 := Struct.Dst_Access_Mask_3;
        
        return MBAF3C;
    end To_C;

    procedure Free(Struct: in out Memory_Barrier_Access_Flags_3_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;
    ...
    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Video_Maintenance_2_Features;
         C_Struct: in Physical_Device_Video_Maintenance_2_Features_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Video_Maintenance_2 :=
            Utilities.To_Ada(C_Struct.Video_Maintenance_2);
    end To_Ada;

    function To_C
        (Struct: in
            Extensions.KHR.Video_Decode_H264_Inline_Session_Parameters_Info)
        return Video_Decode_H264_Inline_Session_Parameters_Info_C is
        VDHISPIC: Video_Decode_H264_Inline_Session_Parameters_Info_C;
    begin
        VDHISPIC.Next := Extension_Records.To_C(Struct.Next);
        VDHISPIC.Std_SPS := Struct.Std_SPS;
        VDHISPIC.Std_PPS := Struct.Std_PPS;

        return VDHISPIC;
    end To_C;

    procedure Free
        (Struct: in out Video_Decode_H264_Inline_Session_Parameters_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;
    ...
    function To_C
        (Struct: in
            Extensions.KHR.Video_Decode_H265_Inline_Session_Parameters_Info)
        return Video_Decode_H265_Inline_Session_Parameters_Info_C is
        VDHISPIC: Video_Decode_H265_Inline_Session_Parameters_Info_C;
    begin
        VDHISPIC.Next := Extension_Records.To_C(Struct.Next);
        VDHISPIC.Std_VPS := Struct.Std_VPS;
        VDHISPIC.Std_SPS := Struct.Std_SPS;
        VDHISPIC.Std_PPS := Struct.Std_PPS;
    ...
    procedure Free
        (Struct: in out Video_Decode_H265_Inline_Session_Parameters_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;
    ...
    function To_C
        (Struct: in
            Extensions.KHR.Video_Decode_AV1_Inline_Session_Parameters_Info)
        return Video_Decode_AV1_Inline_Session_Parameters_Info_C is
        VDAISPIC: Video_Decode_AV1_Inline_Session_Parameters_Info_C;
    begin
        VDAISPIC.Next := Extension_Records.To_C(Struct.Next);
        VDAISPIC.Std_Sequence_Header := Struct.Std_Sequence_Header;

        return VDAISPIC;
    end To_C;

    procedure Free
        (Struct: in out Video_Decode_AV1_Inline_Session_Parameters_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;
    ...
    procedure To_Ada
        (Ada_Struct: in out
            Extensions.KHR.Physical_Device_Depth_Clamp_Zero_One_Features;
         C_Struct: in Physical_Device_Depth_Clamp_Zero_One_Features_C) is
    begin
        Extension_Records.To_Ada(Ada_Struct.Next, C_Struct.Next);
        Ada_Struct.Depth_Clamp_Zero_One :=
            Utilities.To_Ada(C_Struct.Depth_Clamp_Zero_One);
    end To_Ada;

    procedure To_C(C_Address: out Device_Or_Host_Address_C;
                   Ada_Address: in Extensions.KHR.Device_Or_Host_Address) is
    begin
        case Ada_Address.Add_Type is
            when Extensions.KHR.Device =>
                C_Address.Device_Address := Ada_Address.Device_Address;
            when Extensions.KHR.Host =>
                C_Address.Host_Address := Ada_Address.Host_Address;
        end case;
    end To_C;
    ...
    function To_C(Struct: in Extensions.KHR.Android_Surface_Create_Info)
        return Android_Surface_Create_Info_C is
        ASCIC: Android_Surface_Create_Info_C;
    begin
        ASCIC.Next := Extension_Records.To_C(Struct.Next);
        ASCIC.Flags := Struct.Flags;
        ASCIC.Window := Struct.Window;

        return ASCIC;
    end To_C;

    procedure Free(Struct: in out Android_Surface_Create_Info_C) is
    begin
        Extension_Records.Free(Struct.Next);
    end Free;
    ...
    begin
        ...
        case In_Structure(Next.Record_Type) is
            ...
            when Device_Pipeline_Binary_Internal_Cache_Control_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                  (Extensions.KHR.Device_Pipeline_Binary_Internal_Cache_Control,
                   Device_Pipeline_Binary_Internal_Cache_Control_C,
                   Device_Pipeline_Binary_Internal_Cache_Control_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Pipeline_Create_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Pipeline_Create_Info,
                         Pipeline_Create_Info_C,
                         Pipeline_Create_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Pipeline_Binary_Create_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Pipeline_Binary_Create_Info,
                         Pipeline_Binary_Create_Info_C,
                         Pipeline_Binary_Create_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Pipeline_Binary_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Pipeline_Binary_Info,
                         Pipeline_Binary_Info_C,
                         Pipeline_Binary_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Pipeline_Binary_Handles_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Pipeline_Binary_Handles_Info,
                         Pipeline_Binary_Handles_Info_C,
                         Pipeline_Binary_Handles_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            ...
            when Video_Encode_AV1_Session_Create_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Video_Encode_AV1_Session_Create_Info,
                         Video_Encode_AV1_Session_Create_Info_C,
                         Video_Encode_AV1_Session_Create_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_Session_Parameters_Create_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                (Extensions.KHR.Video_Encode_AV1_Session_Parameters_Create_Info,
                 Video_Encode_AV1_Session_Parameters_Create_Info_C,
                 Video_Encode_AV1_Session_Parameters_Create_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_Picture_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Video_Encode_AV1_Picture_Info,
                         Video_Encode_AV1_Picture_Info_C,
                         Video_Encode_AV1_Picture_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_DPB_Slot_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Video_Encode_AV1_DPB_Slot_Info,
                         Video_Encode_AV1_DPB_Slot_Info_C,
                         Video_Encode_AV1_DPB_Slot_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_Profile_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Video_Encode_AV1_Profile_Info,
                         Video_Encode_AV1_Profile_Info_C,
                         Video_Encode_AV1_Profile_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_GOP_Remaining_Frame_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                      (Extensions.KHR.Video_Encode_AV1_GOP_Remaining_Frame_Info,
                       Video_Encode_AV1_GOP_Remaining_Frame_Info_C,
                       Video_Encode_AV1_GOP_Remaining_Frame_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_Rate_Control_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                      (Extensions.KHR.Video_Encode_AV1_Rate_Control_Info,
                       Video_Encode_AV1_Rate_Control_Info_C,
                       Video_Encode_AV1_Rate_Control_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_Rate_Control_Layer_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                      (Extensions.KHR.Video_Encode_AV1_Rate_Control_Layer_Info,
                       Video_Encode_AV1_Rate_Control_Layer_Info_C,
                       Video_Encode_AV1_Rate_Control_Layer_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            ...
            when Video_Encode_Quantization_Map_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Video_Encode_Quantization_Map_Info,
                         Video_Encode_Quantization_Map_Info_C,
                         Video_Encode_Quantization_Map_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when
            Video_Encode_Quantization_Map_Session_Parameters_Create_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
   (Extensions.KHR.Video_Encode_Quantization_Map_Session_Parameters_Create_Info,
    Video_Encode_Quantization_Map_Session_Parameters_Create_Info_C,
    Video_Encode_Quantization_Map_Session_Parameters_Create_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Memory_Barrier_Access_Flags_3_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Memory_Barrier_Access_Flags_3,
                         Memory_Barrier_Access_Flags_3_C,
                         Memory_Barrier_Access_Flags_3_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Decode_H264_Inline_Session_Parameters_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
               (Extensions.KHR.Video_Decode_H264_Inline_Session_Parameters_Info,
                Video_Decode_H264_Inline_Session_Parameters_Info_C,
                Video_Decode_H264_Inline_Session_Parameters_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Decode_H265_Inline_Session_Parameters_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
               (Extensions.KHR.Video_Decode_H265_Inline_Session_Parameters_Info,
                Video_Decode_H265_Inline_Session_Parameters_Info_C,
                Video_Decode_H265_Inline_Session_Parameters_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Decode_AV1_Inline_Session_Parameters_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                (Extensions.KHR.Video_Decode_AV1_Inline_Session_Parameters_Info,
                 Video_Decode_AV1_Inline_Session_Parameters_Info_C,
                 Video_Decode_AV1_Inline_Session_Parameters_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            ...
            when Android_Surface_Create_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_In_Struct
                        (Extensions.KHR.Android_Surface_Create_Info,
                         Android_Surface_Create_Info_C,
                         Android_Surface_Create_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
       ...
       Physical_Device_Workgroup_Memory_Explicit_Layout_Features_C_Access);
          ...
          (Extensions.KHR.Physical_Device_Shader_Maximal_Reconvergence_Features,
           ...
           Physical_Device_Shader_Maximal_Reconvergence_Features_C_Access);
            ...
            when Physical_Device_Pipeline_Binary_Features_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                       (Extensions.KHR.Physical_Device_Pipeline_Binary_Features,
                        Physical_Device_Pipeline_Binary_Features_C,
                        Physical_Device_Pipeline_Binary_Features_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Pipeline_Binary_Properties_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                     (Extensions.KHR.Physical_Device_Pipeline_Binary_Properties,
                      Physical_Device_Pipeline_Binary_Properties_C,
                      Physical_Device_Pipeline_Binary_Properties_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Pipeline_Binary_Key_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                        (Extensions.KHR.Pipeline_Binary_Key,
                         Pipeline_Binary_Key_C,
                         Pipeline_Binary_Key_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Release_Captured_Pipeline_Data_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                        (Extensions.KHR.Release_Captured_Pipeline_Data_Info,
                         Release_Captured_Pipeline_Data_Info_C,
                         Release_Captured_Pipeline_Data_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Pipeline_Binary_Data_Info_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                        (Extensions.KHR.Pipeline_Binary_Data_Info,
                         Pipeline_Binary_Data_Info_C,
                         Pipeline_Binary_Data_Info_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            ...
            when Physical_Device_Compute_Shader_Derivatives_Features_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
            (Extensions.KHR.Physical_Device_Compute_Shader_Derivatives_Features,
             Physical_Device_Compute_Shader_Derivatives_Features_C,
             Physical_Device_Compute_Shader_Derivatives_Features_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Compute_Shader_Derivatives_Properties_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
          (Extensions.KHR.Physical_Device_Compute_Shader_Derivatives_Properties,
           Physical_Device_Compute_Shader_Derivatives_Properties_C,
           Physical_Device_Compute_Shader_Derivatives_Properties_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            ...
            when Physical_Device_Video_Encode_AV1_Features_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                      (Extensions.KHR.Physical_Device_Video_Encode_AV1_Features,
                       Physical_Device_Video_Encode_AV1_Features_C,
                       Physical_Device_Video_Encode_AV1_Features_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_Capabilities_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                        (Extensions.KHR.Video_Encode_AV1_Capabilities,
                         Video_Encode_AV1_Capabilities_C,
                         Video_Encode_AV1_Capabilities_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_Quality_Level_Properties_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                      (Extensions.KHR.Video_Encode_AV1_Quality_Level_Properties,
                       Video_Encode_AV1_Quality_Level_Properties_C,
                       Video_Encode_AV1_Quality_Level_Properties_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            ...
            when Video_Encode_Quantization_Map_Capabilities_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                     (Extensions.KHR.Video_Encode_Quantization_Map_Capabilities,
                      Video_Encode_Quantization_Map_Capabilities_C,
                      Video_Encode_Quantization_Map_Capabilities_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Format_Quantization_Map_Properties_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                       (Extensions.KHR.Video_Format_Quantization_Map_Properties,
                        Video_Format_Quantization_Map_Properties_C,
                        Video_Format_Quantization_Map_Properties_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Video_Encode_Quantization_Map_Features_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
         (Extensions.KHR.Physical_Device_Video_Encode_Quantization_Map_Features,
          Physical_Device_Video_Encode_Quantization_Map_Features_C,
          Physical_Device_Video_Encode_Quantization_Map_Features_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_H264_Quantization_Map_Capabilities_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                (Extensions.KHR.Video_Encode_H264_Quantization_Map_Capabilities,
                 Video_Encode_H264_Quantization_Map_Capabilities_C,
                 Video_Encode_H264_Quantization_Map_Capabilities_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_H265_Quantization_Map_Capabilities_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                (Extensions.KHR.Video_Encode_H265_Quantization_Map_Capabilities,
                 Video_Encode_H265_Quantization_Map_Capabilities_C,
                 Video_Encode_H265_Quantization_Map_Capabilities_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Format_H265_Quantization_Map_Properties_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                  (Extensions.KHR.Video_Format_H265_Quantization_Map_Properties,
                   Video_Format_H265_Quantization_Map_Properties_C,
                   Video_Format_H265_Quantization_Map_Properties_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Encode_AV1_Quantization_Map_Capabilities_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                 (Extensions.KHR.Video_Encode_AV1_Quantization_Map_Capabilities,
                  Video_Encode_AV1_Quantization_Map_Capabilities_C,
                  Video_Encode_AV1_Quantization_Map_Capabilities_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Video_Format_AV1_Quantization_Map_Properties_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                   (Extensions.KHR.Video_Format_AV1_Quantization_Map_Properties,
                    Video_Format_AV1_Quantization_Map_Properties_C,
                    Video_Format_AV1_Quantization_Map_Properties_C_Access);
                begin
                    return Make_Struct(Next);
                end;
       when Physical_Device_Shader_Relaxed_Extended_Instruction_Features_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
   (Extensions.KHR.Physical_Device_Shader_Relaxed_Extended_Instruction_Features,
    Physical_Device_Shader_Relaxed_Extended_Instruction_Features_C,
    Physical_Device_Shader_Relaxed_Extended_Instruction_Features_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Maintenance_7_Features_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                        (Extensions.KHR.Physical_Device_Maintenance_7_Features,
                         Physical_Device_Maintenance_7_Features_C,
                         Physical_Device_Maintenance_7_Features_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Maintenance_7_Properties_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                       (Extensions.KHR.Physical_Device_Maintenance_7_Properties,
                        Physical_Device_Maintenance_7_Properties_C,
                        Physical_Device_Maintenance_7_Properties_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Layered_API_Properties_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                        (Extensions.KHR.Physical_Device_Layered_API_Properties,
                         Physical_Device_Layered_API_Properties_C,
                         Physical_Device_Layered_API_Properties_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Layered_API_Properties_List_Type =>
                declare
                    use type Interfaces.Unsigned_32;

                    function To_Out_Structure_C_Access is
                        new Ada.Unchecked_Conversion
                          (Physical_Device_Layered_API_Properties_List_C_Access,
                           C.Out_Structure_C_Access);
                ...
                begin
                    C_Struct :=
                        new Physical_Device_Layered_API_Properties_List_C;
                    C_Struct.Layered_API_Count := Ada_Struct.Layered_API_Count;

                    if C_Struct.Layered_API_Count /= 0 then
                        C_Struct.Layered_APIs :=
                            Physical_Device_Layered_API_Properties_C_Arrays.
                                Allocate(Positive(C_Struct.Layered_API_Count))
                                    (1)'Access;
                    end if;
            ...
            when Physical_Device_Layered_API_Vulkan_Properties_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                  (Extensions.KHR.Physical_Device_Layered_API_Vulkan_Properties,
                   Physical_Device_Layered_API_Vulkan_Properties_C,
                   Physical_Device_Layered_API_Vulkan_Properties_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Maintenance_8_Features_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                        (Extensions.KHR.Physical_Device_Maintenance_8_Features,
                         Physical_Device_Maintenance_8_Features_C,
                         Physical_Device_Maintenance_8_Features_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Video_Maintenance_2_Features_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                   (Extensions.KHR.Physical_Device_Video_Maintenance_2_Features,
                    Physical_Device_Video_Maintenance_2_Features_C,
                    Physical_Device_Video_Maintenance_2_Features_C_Access);
                begin
                    return Make_Struct(Next);
                end;
            when Physical_Device_Depth_Clamp_Zero_One_Features_Type =>
                declare
                    function Make_Struct is new Utilities.Make_Out_Struct
                  (Extensions.KHR.Physical_Device_Depth_Clamp_Zero_One_Features,
                   Physical_Device_Depth_Clamp_Zero_One_Features_C,
                   Physical_Device_Depth_Clamp_Zero_One_Features_C_Access);
                begin
                    return Make_Struct(Next);
                end;
        end case;
                ...
                declare
                    function To_Access is new Ada.Unchecked_Conversion
                        (C.Out_Structure_C_Access,
                         Pipeline_Binary_Key_C_Access);
                begin
                    To_Ada(Extensions.KHR.Pipeline_Binary_Key(Ada_Struct),
                           To_Access(Next).all);
                end;
            when Release_Captured_Pipeline_Data_Info_Type =>
                declare
                    function To_Access is new Ada.Unchecked_Conversion
                        (C.Out_Structure_C_Access,
                         Release_Captured_Pipeline_Data_Info_C_Access);
                begin
                    To_Ada(Extensions.KHR.Release_Captured_Pipeline_Data_Info
                            (Ada_Struct),
                           To_Access(Next).all);
                end;
            when Pipeline_Binary_Data_Info_Type =>
                declare
                    function To_Access is new Ada.Unchecked_Conversion
                        (C.Out_Structure_C_Access,
                         Pipeline_Binary_Data_Info_C_Access);
                begin
                    To_Ada(Extensions.KHR.Pipeline_Binary_Data_Info
                            (Ada_Struct),
                           To_Access(Next).all);
                end;
            when Cooperative_Matrix_Properties_Type =>
                ...
                declare
                    function To_Access is new Ada.Unchecked_Conversion
                        (C.Out_Structure_C_Access,
                         Physical_Device_Video_Encode_AV1_Features_C_Access);
                begin
                 To_Ada(Extensions.KHR.Physical_Device_Video_Encode_AV1_Features
                         (Ada_Struct),
                        To_Access(Next).all);
                end;
            when Video_Encode_AV1_Capabilities_Type =>
                declare
                    function To_Access is new Ada.Unchecked_Conversion
                        (C.Out_Structure_C_Access,
                         Video_Encode_AV1_Capabilities_C_Access);
                begin
                 To_Ada(Extensions.KHR.Video_Encode_AV1_Capabilities
                         (Ada_Struct),
                        To_Access(Next).all);
                end;
            when Video_Encode_AV1_Quality_Level_Properties_Type =>
                declare
                    function To_Access is new Ada.Unchecked_Conversion
                        (C.Out_Structure_C_Access,
                         Video_Encode_AV1_Quality_Level_Properties_C_Access);
                begin
                 To_Ada(Extensions.KHR.Video_Encode_AV1_Quality_Level_Properties
                         (Ada_Struct),
                        To_Access(Next).all);
                end;
            when Physical_Device_Video_Maintenance_1_Features_Type =>
                ...
                declare
                    function To_Access is new Ada.Unchecked_Conversion
                        (C.Out_Structure_C_Access,
                         Physical_Device_Maintenance_7_Features_C_Access);
                begin
                    To_Ada(Extensions.KHR.Physical_Device_Maintenance_7_Features
                             (Ada_Struct),
                           To_Access(Next).all);
                end;
            when Physical_Device_Maintenance_7_Properties_Type =>
                ...
                declare
                    function To_Access is new Ada.Unchecked_Conversion
                        (C.Out_Structure_C_Access,
                         Physical_Device_Maintenance_8_Features_C_Access);
                begin
                    To_Ada(Extensions.KHR.Physical_Device_Maintenance_8_Features
                            (Ada_Struct),
                           To_Access(Next).all);
                end;
            when Physical_Device_Video_Maintenance_2_Features_Type =>
        ...
        (Physical_Device_Shader_Relaxed_Extended_Instruction_Features_C,
         Physical_Device_Shader_Relaxed_Extended_Instruction_Features_C_Access);
            ...
            when Physical_Device_Layered_API_Properties_List_Type =>
                declare
                    procedure Free_Struct is new Utilities.Free_Out_Struct
                        (Physical_Device_Layered_API_Properties_List_C,
                         Physical_Device_Layered_API_Properties_List_C_Access);
                begin
                    Free_Struct(Next);
                end;
